App =
{
  ...
  renderCandidates: async () => {
    ...
    for (var i = 1; i <= candidateCount; i++) {
      ...
      if (con_id == constituencyId) {
        ...
        var candidateTemplate = "<tr><th><input type='radio' name='ok' id='candidate' value= " + cid + ">" + can_name + "</th><td>" + party_name + "</td><td><img class='rounded-circle' src='./static/" + party_name + ".png' height='100' width='100'></td></tr>"
  ...
  getResults: async () => {
    ...
    for (var i = 1; i <= candidateCount; i++) {
      ...
      const party_name = candidate[5]
      const constituencyId = candidate[3].toNumber()
      console.log("candidates constituencyId:" + constituencyId)
      const constituencyName = candidate[4]
  ...
  generate: async () => {
    ...
    console.log(hash)
    await App.voting.storeHash(hash, { from: web3.eth.accounts[0], gas: 3000000 });
  },
  ...
  voteForCandidate: async () => {
    ...
    if (v[1] == true) {
      ...
      console.log("e:" + e)
      //var candidateName = e;  
      await App.voting.vote(e, { from: web3.eth.accounts[0], gas: 3000000 });
      document.getElementById("canDiv").style.display = "none";
      document.getElementById("thankyou").style.display = "block";
    ...
    docRef.get().then(async (doc) => {
      if (doc.exists) {
        ...
        if (App.qOTP == App.eOTP) {
          await App.voting.registerToVote(vid, { from: web3.eth.accounts[0], gas: 3000000 });
  ...
  checkTime: async () => {
    ...
    if (account != admin_account) alert("You don't have rights to access this option")
      ...
      else {//document.getElementById("results").disabled = true;
  ...
  cancelReg: async () => {
    ...
    if (account != admin_account) alert("You don't have rights to access this option")
      ...
      if (v[1] == true) {
        ...
        await App.voting.cancelReg(vid, { from: web3.eth.accounts[0], gas: 3000000 });
      }
    }
  ...
  setVotingDates: async () => {
    ...
    if (s >= e) alert("Invalid date and time selected")
      ...
      await App.voting.setDates(s / 1000.0, e / 1000.0, { from: web3.eth.accounts[0], gas: 3000000 });
      // App.startDate=svd;
      // App.endDate=evd;
